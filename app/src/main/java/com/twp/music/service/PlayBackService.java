...
public class PlayBackService extends Service {
    ...
    //,Service 状态变更的时候通知页面刷新状态。
    //播放状态改变
    public static final String PLAYSTATE_CHANGED = "com.twp.music.playstatechanged";
    //播放媒体文件改变
    public static final String META_CHANGED = "com.twp.music.metachanged";
    //播放队列改变
    public static final String QUEUE_CHANGED = "com.twp.music.queuechanged";
    ...
    private int mOpenFailedCounter = -1;
    //随机播放获取的下一个position
    private final Shuffler mRand = new Shuffler();
    //当前播放模式标识位
    private PlayMode playModel = PlayMode.NORMAL;
    ...
    //当前服务是否在使用, 有activity在绑定或者重复绑定这个值为true , 解绑的时候为false
    private boolean mServiceInUse = false;

    //service 启动的id  onStartCommand 的时候赋值， 在 stopself(serviceStartId)使用
                ...
                case FOCUSCHANGE:
                    ...
                    switch (msg.arg1) {
                        case AudioManager.AUDIOFOCUS_LOSS:
                            Logger.v("AudioFocus: received AUDIOFOCUS_LOSS");
                            if (isPlaying()) {
                        ...
                        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                            Logger.v("AudioFocus: received AUDIOFOCUS_LOSS_TRANSIENT");
                            if (isPlaying()) {
                        ...
                        case AudioManager.AUDIOFOCUS_GAIN:
                            Logger.v("AudioFocus: received AUDIOFOCUS_GAIN");
                            ...
                            break;
                        default:
                            Logger.e("Unknown audio focus change code");
                    }
                    break;
    ...
    /**
     * 由于播放实现是在Service,所以不知道是那个activity在现实信息，当播放的歌曲信息发生改变后无法更新界面，所以这里通过发送广播来通知
     * 1.Activity
     * 2.锁屏更新
     * 3.weight 更新
     * 目前只处理activty
    ...
    private void notifyChange(String what) {
        ...
        //sendStickyBroadcast，它将发出的广播保存起来，一旦发现有人注册这条广播，则立即能接收到
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Logger.i("mIntentReceiver.onReceive " + action + " / "
                    + cmd);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            String action = intent.getAction();
            String cmd = intent.getStringExtra("command");
            Logger.i("onStartCommand " + action + " / " + cmd);
    ...
    public void open(long[] list, int position) {
        synchronized (this) {
            ...
            //newList=true 表示 传入的list 和之前播放的不一样，如果没有播放是不会走这一步
    ...
    private void saveQueue(boolean full) {
        ...
        ed.putInt("playModel", playModel.getIntValue());
    ...
    private void reloadQueue() {
        ...
        if (qlen > 1) {
            ...
            int shufmode = mPreferences.getInt("playModel", PlayMode.NORMAL.intValue);
            if (shufmode == PlayMode.SHUFFLE.intValue) {
            ...
            playModel = PlayMode.getModel(shufmode);
        }
    }
    ...
    private void openCurrentAndNext() {
        synchronized (this) {
            ...
            while (true) {
                ...
                if (mCursor != null
                        && mCursor.getCount() != 0) {
                    String playUrl = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
    ...
    public void play() {
        ...
        if (mPlayer.isInitialized()) {
            ...
            //duration > 2000表示歌曲时长 大于2s.
            //mPlayer.position() >= duration - 2000 表示播放器播放到最后只剩下 < 2s
            if (playModel != PlayMode.NOCE && duration > 2000
    ...
    private int getNextPosition(boolean force) {
        ...
        switch (playModel) {
            ...
            case SHUFFLE: {
                ...
                //循环去找播放历史中的记录，如果有 numUnplayed --. 同时 tracks[index] = -1,从可播放ids中移除id
     ...
     * 注册Command广播
     *
     * @see PlayBackService#unRegisterCommand()
     */
    ...
    static class ServiceStub extends IPlayBackService.Stub {
        ...
        //构造器中接受的是MediaPlaybackService的对象，所以接口功能实现其实都是依赖于MediaPlaybackServic中的方法
        ServiceStub(PlayBackService service) {
    ...
    private class MultiPlayer {
        ...
        public MultiPlayer() {
            //阻止播放器休眠
            mCurrentMediaPlayer.setWakeMode(PlayBackService.this,
        ...
        private boolean setDataSourceImpl(MediaPlayer player, String path) {
            try {
                ...
                //如果是 content://开头就是表示是读取的Uri地址，而不是一个真实的本地路径
                if (path.startsWith("content://")) {
                    player.setDataSource(PlayBackService.this,
                            Uri.parse(path));
                } else {
        ...
        public void setNextDataSource(String path) {
            ...
            mNextMediaPlayer.setWakeMode(PlayBackService.this,
                    PowerManager.PARTIAL_WAKE_LOCK);
        ...
        public void start() {
            Logger.i("MultiPlayer.start called");
            ...
            public boolean onError(MediaPlayer mp, int what, int extra) {
                switch (what) {
                    case MediaPlayer.MEDIA_ERROR_SERVER_DIED:
                        ...
                        return true;
                    default:
                        Logger.d("MultiPlayer", "Error: " + what + "," + extra);
                        break;
                }
                return false;
            }
    ...
    /**
     * 查找android.media.MediaPlayer中是否支持  setNextMediaPlayer 这个方法，如果支持
     * 现在您可以在两个完全独立的[[MediaPlayer]]类上执行无缝播放。
     * 在您第一个[[MediaPlayer]]类执行结束前的任何时间调用[[setNextMediaPlayer()]]这个方法，
     * 然后Android系统将会在您第一个停止的时候紧接着播放第二个文件。
     *
     * @author Royal
    ...
    static class CompatMediaPlayer extends MediaPlayer implements
        ...
        //这个方法回调只会是 系统没有 setNextMediaPlayer 这个方法 的时候才会回调
        @Override
        public void onCompletion(MediaPlayer mp) {
            if (mNextPlayer != null) {
