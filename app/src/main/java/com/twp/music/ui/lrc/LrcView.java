...
public class LrcView extends View implements ILrcView{
	...
	private int mHignlightRowColor = Color.parseColor("#03a9f5");
	private int mNormalRowColor = Color.parseColor("#747474");
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			Logger.d(TAG,"down,mLastMotionY:"+mLastMotionY);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if(event.getPointerCount() == 2){
				Logger.d(TAG, "two move");
				doScale(event);
				return true;
			}
			Logger.d(TAG, "one move");
	...
	private void doScale(MotionEvent event) {
		if(mDisplayMode == DISPLAY_MODE_SEEK){
			...
			Logger.d(TAG, "two move but teaking ...change mode");
			return;
		}
		...
		Logger.d(TAG,"scaleSize:" + scaleSize);
		if(scaleSize != 0){
	...
	private void doSeek(MotionEvent event) {
		...
		Logger.d(TAG, "move new hightlightrow : " + mHignlightRow + " offsetY: " + offsetY + " rowOffset:" + rowOffset);
		if(offsetY < 0){
			// finger move up
	...
	private int getScale(MotionEvent event){
		Logger.d(TAG,"scaleSize getScale");
		...
		Logger.d(TAG,"scaleSize maxOffset:" + maxOffset);
    ...
    public void seekLrcToTime(long time) {
        ...
        Logger.d(TAG, "seekLrcToTime:" + time);
        // find row
